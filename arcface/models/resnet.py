...
def bottleneck_block(filters=64, expantion=1, kernel_size=(3, 3),
                     downsample=False, padding='same', activation='relu',
                     kernel_regularizer=None):
...
def bottleneck_blocks(num_blocks=3, filters=64, downsample=False,
...
def rsn_head(output_size, kernel_regularizer=None):
    block_layers = [layers.GlobalAveragePooling2D(),
                    layers.Dense(output_size, kernel_regularizer=kernel_regularizer)]
...
def make_resnet(block_size_list, bottleneck=False, input_shape=(256, 256, 3),
                output_size=1000, kernel_regularizer=None, name='ResNet'):
    ...
    # Conv1
    x = rsn_stem(filters=64, kernel_size=(7, 7), strides=(2, 2),
    ...
    # Output head.
    outputs = rsn_head(output_size, kernel_regularizer)(x)
