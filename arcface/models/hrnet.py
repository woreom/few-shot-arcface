...
from arcface.models.resnet import (BottleneckBlock, ResidualBlock, bottleneck_block,
                           residual_block)


def hrn_1st_stage(filters=64, activation='relu', kernel_regularizer=None):
...
def hrn_block(filters=64, activation='relu', kernel_regularizer=None):
...
def hrn_blocks(repeat=1, filters=64, activation='relu', kernel_regularizer=None):
    block_layers = [hrn_block(filters,
...
def fusion_layer(filters, upsample=False, activation='relu', kernel_regularizer=None):
    block_layers = []
    if upsample:
...
def fusion_block(filters, branches_in, branches_out, activation='relu', kernel_regularizer=None):
        ...
        for column in range(columns):
            if column == row:
                ...
                _fusion_layers.append(fusion_layer(filters * pow(2, column),
                                                   False, activation, kernel_regularizer))
            else:
                # Up sampling.
                _fusion_layers.append(fusion_layer(filters * pow(2, column),
                                                   True, activation, kernel_regularizer))
...
def hrnet_body(filters=64, kernel_regularizer=None):
    # Stage 1
    s1_b1 = hrn_1st_stage(filters, kernel_regularizer=kernel_regularizer)
    s1_fusion = fusion_block(filters, branches_in=1, branches_out=2,
                             kernel_regularizer=kernel_regularizer)

    # Stage 2
    s2_b1 = hrn_block(filters, kernel_regularizer=kernel_regularizer)
    s2_b2 = hrn_block(filters*2, kernel_regularizer=kernel_regularizer)
    s2_fusion = fusion_block(filters, branches_in=2, branches_out=3,
                             kernel_regularizer=kernel_regularizer)

    # Stage 3
    s3_b1 = hrn_blocks(4, filters, kernel_regularizer=kernel_regularizer)
    s3_b2 = hrn_blocks(4, filters*2, kernel_regularizer=kernel_regularizer)
    s3_b3 = hrn_blocks(4, filters*4, kernel_regularizer=kernel_regularizer)
    s3_fusion = fusion_block(filters, branches_in=3, branches_out=4,
                             kernel_regularizer=kernel_regularizer)

    # Stage 4
    s4_b1 = hrn_blocks(3, filters, kernel_regularizer=kernel_regularizer)
    s4_b2 = hrn_blocks(3, filters*2, kernel_regularizer=kernel_regularizer)
    s4_b3 = hrn_blocks(3, filters*4, kernel_regularizer=kernel_regularizer)
    s4_b4 = hrn_blocks(3, filters*8, kernel_regularizer=kernel_regularizer)
